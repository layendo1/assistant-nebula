(function() {
    ...
    const character = {
        ...
        getInstruction: function(profileContext, name) {
            return `Tu es "${name}", l'assistant IA bienveillant, pédagogue et passionné de la plateforme Nebula. Ta personnalité est chaleureuse, encourageante et très patiente. Tu es là pour guider tous les utilisateurs, qu'ils soient enseignants, élèves ou parents.
${profileContext}
...
7.  **Adaptabilité :** Utilise un langage clair et adapté à ton interlocuteur (simple pour un élève, plus technique pour un enseignant).`;
    ...
    function stopAmbientSound() {
        // Empty to disable ambient sounds
    }
    ...
    async function handleTextToSpeech(messageElement, button) {
        ...
        const API_KEY = "AIzaSyA5uH0hPssBKyM6bIZ_aoHPbqikTmSvUHY";
    ...
    async function handleFormSubmit(event) {
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                aiMessageElement.innerHTML = fullResponse + '<span class="typing-cursor"></span>';
                chatHistory.scrollTop = chatHistory.scrollHeight;
            }

            // Final render pass with full content
            aiMessageElement.innerHTML = fullResponse;
    ...
    async function handleSummarizeClick(url, container) {
        ...
        try {
            ...
            for await (const chunk of stream) {
                fullResponse += chunk.text;
                resultContainer.innerHTML = fullResponse;
            }
        } catch (e) {
    ...
    function loadSettings() {
        ...
        const soundThemeInput = document.querySelector(`input[name="sound-theme"][value="${settings.soundTheme}"]`);
        if (soundThemeInput) soundThemeInput.setAttribute('checked', 'true');
    ...
    async function renderSuggestions() {
        ...
        if (chatInput.value.trim() !== '' || !loadingIndicator.classList.contains('hidden')) {
            suggestionChipsContainer.classList.add('hidden');
            suggestionChipsContainer.innerHTML = '';
            return;
        }
    ...
    async function main() {
        ...
        if (!API_KEY) {
            ...
            // Note: The GoogleGenAI class would need to be available globally or defined separately
            // For now, we'll assume it's available via a script tag
